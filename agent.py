...
#  اجرا روی: VPS خارج (نود)  |  localhost:8765  |  پشت Nginx
#  نصب: pip install fastapi uvicorn psutil
# ============================================================
...
def auth(key: str, request: Request):
    if key != API_KEY:
        log.warning(f"Unauthorized from {request.client.host}")
...
def collect_network() -> dict:
    ...
    try:
        ...
        if m:
            ping_ms = float(m.group(2)); jitter = float(m.group(4))
...
if __name__ == "__main__":
    ...
    log.info(f"Node agent starting on {AGENT_HOST}:{AGENT_PORT}")
